...
int main(int argc, char** argv) {
  ...
  //Input Initialization
    ...
    {
        omp_set_nested(1);
        #if defined(PONE) || defined(PTHREE) || defined(EFFPAR) || defined(OPT)
        {
            #pragma omp parallel
            {
                long p = N;
                #pragma omp single
                MMScanDNC(X, Y, Temp, 0, N-1, B, p, p); // last param is from command line and the one before that is arbitrary
            }
        }
        #else
        {
            long p = N;
            MMScanDNC(X, Y, Temp, 0, N-1, B, p, p);
        }
        #endif
    }
    #else
